...
function buildSvg({ width, height, label, ratio }) {
...
const targets = [
  // Visuel vertical du Hero
  { path: 'src/assets/placeholders/hero.jpg', width: 1200, height: 1600, label: 'VISUEL HERO', ratio: '3 : 4 — VERTICAL' },
...
for (const { path, width, height, label, ratio } of targets) {
  const out = resolve(process.cwd(), path);
  await mkdir(dirname(out), { recursive: true });
  await sharp(Buffer.from(buildSvg({ width, height, label, ratio })))
